L.Control.HtmlLegend = L.Control.extend({
    ...
    _updateOpacityStroke(layer, opacity_stroke) {
        if (typeof this.options.updateOpacityStroke === 'function') {
            this.options.updateOpacityStroke(layer, opacity_stroke)
        } else if (typeof layer.setOpacity === 'function') {
            layer.setOpacity(opacity_stroke);
        } else if (typeof layer.setStyle === 'function') {
            layer.setStyle({ opacity_stroke });
        }
    },
        ...
        if (!this.options.disableVisibilityControls) {
            ...
            L.DomEvent.on(toggleButton, 'click', (e) => {
                ...
                if (L.DomUtil.hasClass(button, 'disabled')) {
                    ...
                    this._updateOpacityStroke(layer, button.dataset.visibileOpacity);
                }
                else {
                    L.DomUtil.addClass(button, 'disabled');
                    this._updateOpacity(layer, 0);
                    this._updateOpacityStroke(layer, 0);
                }
            });
            ...
            //////MY CHANGES HERE
            const opacitySlider2 = L.DomUtil.create('input', null, opacityController);
            opacitySlider2.type = 'range';
            opacitySlider2.min = 0;
            opacitySlider2.max = 1;
            opacitySlider2.step = 0.1;
            opacitySlider2.onchange = ((e) => {
                const newOpacity2 = 1 - e.target.value || 0;
                this._updateOpacityStroke(layer, newOpacity2);
                toggleButton.dataset.visibileOpacity = newOpacity2;
                L.DomUtil.removeClass(toggleButton, 'disabled');
            });
            opacitySlider2.value = 1 - (opacity);
        };
